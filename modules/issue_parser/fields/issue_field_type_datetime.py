...
class IssueFieldTypeDatetime(IssueFieldType):
    ...
    def __init__(self, field_name: str, jira_field_name: str):
        super().__init__(field_name, jira_field_name)
        self.__return_pattern = IssueFieldTypeDate.DEFAULT_RETURN_PATTERN
        self.__target_time_zone = pytz.timezone(IssueFieldTypeDate.DEFAULT_TIME_ZONE)
    ...
    def data(self) -> str:
        ...
        try:
            ...
            if self.__return_pattern == IssueFieldTypeDate.RETURN_DATE_TIME:
                return dt_converted.strftime("%Y-%m-%d %H:%M:%S %z")
            elif self.__return_pattern == IssueFieldTypeDate.RETURN_UNIX_TIMESTAMP:
                return str(int(dt_converted.timestamp()))
            elif self.__return_pattern == IssueFieldTypeDate.RETURN_UNIX_TIMESTAMP_MILLIS:
    ...
    def data(
        ...
        if value is None or not isinstance(value, str) or len(value) == 0:
            ...
            try:
                self._data = datetime.strptime(value, IssueFieldTypeDate.DATE_PATTERN)
            except Exception:
                raise Exception(f"Failed to parse date value '{self._value}' with pattern '{IssueFieldTypeDate.DATE_PATTERN}'")
        else:
    ...
    def return_pattern(self, value: int) -> None:
        if value in IssueFieldTypeDate.RETURN_PATTERN_OPTIONS:
            self.__return_pattern = value
        else:
            self.__return_pattern = IssueFieldTypeDate.DEFAULT_RETURN_PATTERN
            raise ValueError(f"The given return pattern '{value}' is invalid. Valid options are: {IssueFieldTypeDate.RETURN_PATTERN_OPTIONS}. The return pattern has been set to the default value '{IssueFieldTypeDate.DEFAULT_RETURN_PATTERN}'.")
    ...
    def target_time_zone(self, value: str) -> None:
        ...
        try:
            ...
            self.__target_time_zone = pytz.timezone(IssueFieldTypeDate.DEFAULT_TIME_ZONE)
            raise ValueError(f"The configured time zone '{value}' is invalid. Please check the configuration. Falling back to '{IssueFieldTypeDate.DEFAULT_TIME_ZONE}'.")
